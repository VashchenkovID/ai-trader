...
- **Метод**: `performActiveSignalsPricesUpdate()`
- **Расписание**: Каждые 5 минут (`*/5 * * * *`)
- **Файл**: `server/src/services/SchedulerService.js`
- **Описание**: Обновляет цены для активных торговых сигналов
- **Приоритет**: Высокий (торговые операции)

#### 2. Обновление цен торговых заявок
- **Метод**: `performTradingRequestsPricesUpdate()`
- **Расписание**: Каждую минуту (настраивается)
- **Файл**: `server/src/services/SchedulerService.js`
- **Описание**: Обновляет цены для активных торговых заявок
- **Приоритет**: Высокий (критично для торговли)

#### 3. Ежедневное обновление новостей
- **Метод**: `performDailyNewsUpdate()`
- **Расписание**: Каждый день в 9:00 (`0 9 * * *`)
- **Файл**: `server/src/services/SchedulerService.js`
- **Описание**: Загружает свежие новости для всех инструментов с ротацией (30 инструментов в день)
- **Приоритет**: Высокий (влияет на анализ)

#### 4. Еженедельная очистка новостей
- **Метод**: `performNewsCacheCleanup()`
- **Расписание**: Каждое воскресенье в 3:00 (`0 3 * * 0`)
- **Файл**: `server/src/services/SchedulerService.js`
- **Описание**: Удаляет новости старше года
- **Приоритет**: Средний

#### 5. Обновление кеша Telegram
- **Метод**: `performTelegramCacheUpdate()`
- **Расписание**: Каждые 6 часов (`0 */6 * * *`)
- **Файл**: `server/src/services/SchedulerService.js`
- **Описание**: Обновляет кеш настроений из Telegram
- **Приоритет**: Средний
