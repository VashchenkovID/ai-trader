"""
Провайдер DeepSeek API для LLM-жюри.

Использует https://api.deepseek.com/chat/completions (OpenAI-совместимый формат).
...
class DeepSeekProvider(LLMProviderBase):
    """
    Провайдер DeepSeek. Требует DEEPSEEK_API_KEY.
    ...
    def __init__(
        ...
        raw_key = os.environ.get("DEEPSEEK_API_KEY") if api_key is None else api_key
...
def _extract_content(data: dict[str, Any]) -> str:
    """Извлекает текст ответа из JSON (OpenAI-формат)."""
