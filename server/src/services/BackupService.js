...
class BackupService {
        ...
        try {
            ...
            try {
                ...
                if (LoggerService.isInitialized) {
                    LoggerService.error('❌ Ошибка бэкапа настроек:', {error});
                }
            ...
            try {
                ...
                if (LoggerService.isInitialized) {
                    LoggerService.error('❌ Ошибка бэкапа моделей:', error);
                }
            ...
            await fs.writeFile(metadataPath, JSON.stringify(results, null, 2));
            if (LoggerService.isInitialized) {
                LoggerService.info(`✅ Полный бэкап создан: ${backupId}`);
            }
            ...
            if (LoggerService.isInitialized) {
                LoggerService.error('❌ Ошибка создания полного бэкапа:', {error});
            }
    ...
    async backupDatabase(backupId) {
        ...
        try {
            ...
            if (dbConfig.dialect === 'postgres') {
                ...
                if (pgDumpAvailable) {
                    ...
                    try {
                        ...
                        if (LoggerService.isInitialized) {
                            LoggerService.warn('⚠️ pg_dump не удался, используем альтернативный метод:', {execError});
            ...
            if (LoggerService.isInitialized) {
                LoggerService.error('❌ Ошибка бэкапа БД:', {error});
            }
            return {
                success: false,
                error: error.message,
    ...
    async backupDatabaseViaSequelize(backupPath, backupFileName) {
        try {
            ...
            if (LoggerService.isInitialized) {
                LoggerService.error('❌ Ошибка бэкапа БД через Sequelize:', {error});
            }
            throw error;
        }
    }
            ...
            for (const modelName of modelFiles) {
                try {
                    ...
                    if (LoggerService.isInitialized) {
                        ...
                        LoggerService.warn(`⚠️ Модель ${modelName} не найдена, пропускаем`);
                    }
                }
            }
        } catch (error) {
            if (LoggerService.isInitialized) {
                LoggerService.warn('⚠️ Ошибка загрузки моделей:');
            }
        }
    ...
    async listBackups(type = 'all') {
        try {
            ...
            if (LoggerService.isInitialized) {
                LoggerService.error('❌ Ошибка получения списка бэкапов:', {error});
            }
            throw error;
        }
    }
    ...
    async getDirectorySize(dir) {
        ...
        try {
            ...
            if (LoggerService.isInitialized) {
                LoggerService.warn(`⚠️ Не удалось вычислить размер ${dir}:`, {error});
            }
        }
        return size;
    }
    ...
    async getBackupInfo(backupId) {
        try {
            ...
            if (LoggerService.isInitialized) {
                LoggerService.error('❌ Ошибка получения информации о бэкапе:', {error});
            }
            throw error;
        }
    }
    ...
    async restoreBackup(backupId, options = {}) {
        ...
        try {
            ...
            if (LoggerService.isInitialized) {
                LoggerService.error('❌ Ошибка восстановления из бэкапа:', {error});
            }
            throw error;
        }
    }
    ...
    async restoreDatabase(backupPath) {
        ...
        try {
            ...
            if (LoggerService.isInitialized) {
                LoggerService.error('❌ Ошибка восстановления БД:', {error});
            }
            throw error;
        }
    }
