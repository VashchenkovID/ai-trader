 ...
 * 
 * ВАЖНО: После рефакторинга логирования:
 * - Поддерживаются только методы warn() и error()
 * - Методы info(), debug(), log(), verbose() удалены (no-op)
 * - Все логи пишутся ТОЛЬКО в файлы (combined.log и error.log)
 * - Логи НЕ выводятся в консоль (кроме fallback при ошибках инициализации)
 * 
 * Использование:
 * - LoggerService.warn('Предупреждение', { service: 'ServiceName', operation: 'operationName' })
 * - LoggerService.error('Ошибка', { service: 'ServiceName', operation: 'operationName', error: { message, stack } })
 */
class LoggerService {
    constructor() {
        ...
        try {
            ...
            const transports = [
                ...
                new winston.transports.File({
                    ...
                    format: logFormat,
                    level: 'warn', // Только warn и error (info/debug/log удалены)
            ...
            // ВАЖНО: Логи пишутся ТОЛЬКО в файлы, не в консоль
            // Убрали Console transport согласно требованиям рефакторинга
            // Все логи (warn и error) будут только в файлах

            // Определяем уровень логирования
            // ВАЖНО: Теперь логируем только warn и error (в файлы)
            // Методы info, debug, log удалены согласно рефакторингу
            const isProduction = process.env.NODE_ENV === 'production';
            const logLevel = process.env.LOG_LEVEL || 'warn'; // Всегда только warn и error
            ...
            // Используем warn вместо info, так как info больше не используется
            this.warn('LoggerService initialized', {
            ...
            // В случае ошибки инициализации используем console.error как fallback
            // так как LoggerService еще не инициализирован
    ...
    child(context = {}) {
        if (!this.isInitialized) {
            // Fallback на console если сервис не инициализирован (только для error и warn)
            return {
                error: (...args) => console.error(...args),
                warn: (...args) => console.warn(...args),
                // Методы info, debug, log, verbose удалены - возвращаем no-op функции
                info: () => {},
                debug: () => {},
                log: () => {},
                verbose: () => {},
    ...
    async _log(level, message, meta = {}) {
        ...
        // ВАЖНО: Методы info, debug, log, verbose больше не поддерживаются
        // Если вызывается недопустимый уровень - игнорируем
        if (level !== 'warn' && level !== 'error') {
            // Игнорируем вызовы info, debug, log, verbose
            return;
        }
        ...
        if (!this.isInitialized) {
            ...
            const consoleMethod = level === 'error' ? console.error : 
                                 level === 'warn' ? console.warn : undefined;
        ...
        try {
            ...
            // Предупреждение логируем через LoggerService.warn (если инициализирован)
            if (level === 'error' && this.isInitialized) {
                // Используем прямой вызов logger, чтобы избежать рекурсии
                try {
                    this.logger.warn('SecretManagementService не доступен для маскирования секретов', {
                        service: 'LoggerService',
                        operation: '_log'
                    });
                } catch (logError) {
                    // Игнорируем ошибки логирования предупреждения
                }
            }
        }
    ...
    /**
     * Логирование ошибки
     * Логи пишутся ТОЛЬКО в файлы (combined.log и error.log)
     */
    error(message, meta = {}) {
        this._log('error', message, meta).catch(err => {
            // В случае ошибки логирования используем console.error как последний fallback
     ...
     * Логи пишутся ТОЛЬКО в файлы (combined.log)
     */
    warn(message, meta = {}) {
     ...
     * @deprecated Метод удален согласно рефакторингу логирования
     * Используйте LoggerService.warn() для важных сообщений или удалите вызов
     */
    info(message, meta = {}) {
        // No-op: метод удален, логирование информации больше не поддерживается
        // Все вызовы LoggerService.info() должны быть удалены из кода
    }
     ...
     */
    debug(message, meta = {}) {
        // No-op: метод удален, логирование отладки больше не поддерживается
        // Все вызовы LoggerService.debug() должны быть удалены из кода
    }
    ...
    verbose(message, meta = {}) {
        // No-op: метод удален, логирование подробной информации больше не поддерживается
        // Все вызовы LoggerService.verbose() должны быть удалены из кода
    }

    /**
     * Логирование (алиас для info)
     * @deprecated Метод удален согласно рефакторингу логирования
     * Используйте LoggerService.warn() для важных сообщений или удалите вызов
     */
    log(message, meta = {}) {
        // No-op: метод удален, логирование больше не поддерживается
        // Все вызовы LoggerService.log() должны быть удалены из кода
    }
    ...
    logRequest(req, res, duration = null) {
        ...
        // ВАЖНО: Логируем только ошибки и предупреждения (warn и error)
        // Успешные запросы (2xx, 3xx) больше не логируются
        const level = statusCode >= 500 ? 'error' :
            statusCode >= 400 ? 'warn' : null;
     ...
     * @deprecated Метод больше не логирует операции БД (debug удален)
     * Используйте LoggerService.warn() для важных операций БД или удалите вызов
     */
    logDatabase(operation, meta = {}) {
        // No-op: метод больше не логирует операции БД
        // Все вызовы LoggerService.logDatabase() должны быть удалены или заменены на warn/error
    }

    /**
     * Логирование операции с внешним API
     * ВАЖНО: Логируем только медленные запросы (warn) и ошибки (error)
     */
    logApiCall(service, endpoint, method, duration = null, meta = {}) {
        if (!this.isInitialized) return;

        // Логируем только медленные запросы (warn) и ошибки (error)
        // Быстрые успешные запросы больше не логируются
        const level = duration && duration > 5000 ? 'warn' : null;
        
        // Пропускаем быстрые успешные запросы
        if (level === null) {
            return;
        }
